...
export const updateMetric = async (params: {
  ...
  const {id, ...data} = params.metric

  const result = await fetch(`https://api.datadoghq.com/api/v2/logs/config/metrics/${id}`, {
    method: 'PATCH',
    headers: {
      ...params.config,
  ...
  if (response.errors && response.errors.length > 0) {
    throw new TypeError(`Cannot update metric: ${response.errors[0]}`)
  }
